...
import { HABIT_RISK } from '../game/settle.js';
...
export function usageLabel(state, actorId, itemId) {
  ...
  return usage ? `${NAMES[actorId]}今日 ${usage.used}${usage.limit ? '/' + usage.limit : ''}${usage.unit}` : '';
}
...
export function showQuickItems(actorId, onInventory, onPlan) {
  ...
  showModal(NAMES[actorId], `<p>香烟 ${actor.smokes || 0}次${(actor.smokes || 0) > HABIT_RISK.smokeSafePerDay ? '（过量，夜里可能生病）' : ''} · 喝酒 ${actor.drinks || 0}次 · 醉意 ${actor.intox || 0}/2${(actor.intox || 0) >= 2 ? '（醉了，明天可能不舒服）' : ''} · 今日咖啡 ${s.daily.coffeeCups?.[actorId] || 0}杯</p><div class="quick-items">${items.map(item => {
