...
test('21点合作奖金归原日，次日现金基线与账本同步且不计自动队友收入', () => {
  ...
  assert.equal(done.bonus, 8);
  assert.equal(done.state.cash, 113);
  assert.equal(done.state.lastDayReport.endCash, 113);
  assert.equal(done.state.lastDayReport.cashDelta, 41);
  assert.equal(done.state.lastDayReport.controlledBonusIncome, 8);
  assert.match(renderDayReport(done.state.lastDayReport), /主控小游戏奖金 \+8元/);
  assert.equal(done.state.lastDayReport.totalAutoIncome, 33);
  assert.equal(done.state.daily.report.startCash, 113);
  assert.deepEqual(done.state.ledger, { start: 113, income: 0, expense: 0 });
  assert.equal(validateDayReport(done.state), true);
  assert.equal(state.lastDayReport.endCash, 105);
  assert.equal(state.daily.report.startCash, 105);
